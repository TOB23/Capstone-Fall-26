...
collector.py         one polite pass over the registry -> a timestamped snapshot
  ...
  └── countyjson.py   county-name JSON feed adapter (e.g. FPL)
county_fips.py       county NAME -> FIPS resolver (built from the Census shapefile)
...
batch_check.py       probe every starter URL at once -> a triage worklist
...
notebooks/           the hands-on VS Code surface (these import the modules above)
  ...
  └── 03_url_triage.ipynb            batch-probe starter URLs into a worklist
```
...
Each utility's outage map runs on a vendor platform; one adapter handles all
utilities on that platform. Currently supported:

- **kubra** — KUBRA Storm Center, the most common US vendor. Tiled JSON,
  quadkey descent.
- **arcgis** — ArcGIS FeatureServer/MapServer layers. A REST `/query` endpoint.
- **countyjson** — a single JSON document already aggregated by county name
  (FPL uses this). County names are resolved to FIPS via `county_fips.py`.

Adding a platform is one new `OutageAdapter` subclass registered in
`collector.ADAPTERS`.
...
(`cb_2025_us_county_500k`) unzipped into `data/counties/`. So does
`county_fips.py` (it builds the county-name lookup from that shapefile).

**Working interactively in VS Code:** open the `notebooks/` folder and run them
in order — `00_setup.ipynb` to verify the environment, `03_url_triage.ipynb` to
batch-probe the starter URLs into a worklist, `01_build_registry.ipynb` to
discover, test, and register utilities, and `02_aggregate_and_verify.ipynb` to
aggregate to county and cross-check against EAGLE-I with maps. The notebooks
import the modules above, so the command-line steps and the notebooks do the
same work either way. The numbering is this repo's own sequence —
